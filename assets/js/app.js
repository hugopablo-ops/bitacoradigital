(() => {
  // Evita doble ejecución si por error se carga dos veces el script
  if (window.__BD_LOADED__) return;
  window.__BD_LOADED__ = true;

  const PROXY = String(window.__BD_PROXY || '').trim(); // definido en index.html
  ...
  async function fetchStooqMonthly(ticker){
    if (!PROXY) throw new Error('Proxy no configurado (window.__BD_PROXY)');
    const real = `https://stooq.com/q/d/l/?s=${ticker}&i=m`;
    const url  = PROXY + encodeURIComponent(real);
  ...
  async function drawChile(){
    ...
    // Mensaje si falta proxy
    if (!PROXY) {
      root.innerHTML = '<div style="padding:1rem;color:#a0aec0">Configura window.__BD_PROXY en index.html</div>';
    ...
    try{
      ...
      let [a,b,c] = intersectDates([ufRaw, usdRaw, ech]); // mismo rango
      ...
      // click → detalle
      if (card) card.onclick = () => { window.location.href = '/detail/tradfi-cl'; };
