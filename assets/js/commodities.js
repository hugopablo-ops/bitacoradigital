// Commodities - Oro, Plata, Cobre, Litio (YTD Base100)
(function() {
  'use strict';

  const SERIES = {
    gold: { name: 'Oro', color: '#fbbf24', ticker: 'iau.us' },
    silver: { name: 'Plata', color: '#9ca3af', ticker: 'slv.us' },
    copper: { name: 'Cobre', color: '#f97316', ticker: 'copx.us' },
    lithium: { name: 'Litio', color: '#06b6d4', ticker: 'lit.us' }
  };
  ...
  async function init() {
    ...
    if (!container) {
      console.error('❌ No se encontró #c-commodities');
      return;
    }
  ...
  function hideLoading() {
    const loading = document.querySelector('#c-commodities .bd-loading');
  ...
  // Click handler
  document.getElementById('card-commodities')?.addEventListener('click', () => {
